...
try
{
    if (!std::filesystem::exists(settings::kFilesDir)) 
    {
        ...
        {
            logger_s::ErrorDatabaseCreateDir(settings::kFilesDir.string());
            return;
        }
    }

    SQLite::Database db{settings::server::kDatabaseFile
...
try
{
    db_read_write_ = std::make_shared<SQLite::Database>(settings::server::kDatabaseFile
        , SQLite::OPEN_READWRITE | SQLite::OPEN_SHAREDCACHE);
