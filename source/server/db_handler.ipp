...
try 
{
    ...
    {
        logger_s::ErrorSqlExecuteStep(event->GetType());
...
try
{
    ...
    std::shared_ptr<std::vector<PairIntStr>> avatars = std::make_shared<std::vector<PairIntStr>>();

    while (query.executeStep())
    {
        avatars->push_back(PairIntStr{query.getColumn(id_column).getInt(), query.getColumn(name_column).getText()});
    }
...
try
{
    ...
    event_queue_.AddEvent(std::make_shared<AvatarDataReady>(event->GetSessionId(), std::move(avatar_ptr)));
}
catch (const std::exception& e)
{
    MVP_LOG_EXEPTION(e);
