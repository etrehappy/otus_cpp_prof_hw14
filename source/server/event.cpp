...
LogPasClientResponse::LogPasClientResponse(const SharedMsg m, const std::string& session_id)
    :Event{EventPriority::kMedium
            , EventType::kUserLogPasClientResponse
            , settings::server::kDatabaseEventRoutingKey}    
    , message_{m}, session_id_(session_id)
{
}

const SharedMsg LogPasClientResponse::GetEventMessage() const
{
    return message_;
}
...
SuccessCheckLogPas::SuccessCheckLogPas(const std::string& session_id
        , std::shared_ptr<const std::string> uuid)
    : Event{EventPriority::kMedium, EventType::kSuccessCheckLogPas, session_id}
    , uuid_{uuid}
{
}
...
AvatarListClientRequest::AvatarListClientRequest(const std::string& session_id
        , const std::string& uuid)
    :Event{EventPriority::kMedium, EventType::kUsersAvatarListClientRequest
        , settings::server::kDatabaseEventRoutingKey}
    , session_id_(session_id), uuid_(uuid)
{
}
...
AvatarListIsReady::AvatarListIsReady(const std::shared_ptr<std::vector<PairIntStr>> a
        , const std::string& session_id)
    :Event{EventPriority::kMedium, EventType::kAvatarListIsReady, session_id}
    , avatars_{a}
{
}
...
AvatarDataReady::AvatarDataReady(const std::string& session_id, std::shared_ptr<Avatar> a)
    : Event{EventPriority::kHigh, EventType::kAvatarsDataReady, session_id}, avatar_{a}
{
}

std::shared_ptr<Avatar> AvatarDataReady::MoveAvatar()
{    
...
GetAvatarsData::GetAvatarsData(const std::string& session_id, const uint32_t id_selected_avatar)
    : Event{EventPriority::kMedium
            ...
            , settings::server::kDatabaseEventRoutingKey}
    , avatar_id_{id_selected_avatar}, session_id_{session_id}
{
}
...
StartGameServerResponseSucsses::StartGameServerResponseSucsses(const std::string& session_id
        , const std::shared_ptr<Avatar> wp)
    :Event{EventPriority::kMedium, EventType::kStartGameServerResponseSucsses, session_id}
    , avatar_{wp}
{
}
...
AddItemInInventory::AddItemInInventory(ItemType item, uint32_t avatar_id)
    :Event{EventPriority::kMedium, EventType::kAddItemInInventory
            , settings::server::kDatabaseEventRoutingKey}
    , item_{item}, avatar_id_{avatar_id}
{
}
...
BroadcastMessage::BroadcastMessage(const SharedMsg m, const std::string& session_id)
    :Event{EventPriority::kMedium, EventType::kBroadcastMessage, session_id}
    , message_{m}
{
}

const SharedMsg BroadcastMessage::GetEventMessage() const
{
    return message_;
}

ExitGameRequest::ExitGameRequest(const SharedMsg m, const std::string& session_id)
    :Event{EventPriority::kEndGame, EventType::kExitGameRequest, session_id}, message_{m}
{
}

const SharedMsg ExitGameRequest::GetEventMessage() const
{
    return message_;
}

SaveGame::SaveGame(const std::string& s, const std::shared_ptr<Avatar> a, bool e)
    :Event{EventPriority::kMedium, EventType::kSaveGame
            , settings::server::kDatabaseEventRoutingKey}
