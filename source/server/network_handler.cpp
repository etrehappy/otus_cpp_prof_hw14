...
NetworkHandler::NetworkHandler(std::shared_ptr<BstTcpSocket> s, EventQueue& q, const std::string& session_id)
    :socket_{s}, event_queue_{q}, event_routing_key_{session_id}, uuid_{}, current_avatar_{}
{
}
...
boost::asio::awaitable<void> NetworkHandler::ForwardEventToHandler(ShrdEvent event)
{
try
{
...
{	
    ...
    try
    {
        ...
        SharedMsg message = std::make_shared<ProtoMessage::GameMessage>();
    ...
    catch (const boost::system::system_error& e)
    {
        if (e.code() == boost::asio::error::eof || e.code() == boost::asio::error::connection_reset) 
        {
            ...
            logger_s::ClientDisconnect(client_ip);
...
try
{
    ...
    {
        logger_s::ErrorSerrialisation(type);
        co_return;
    }
