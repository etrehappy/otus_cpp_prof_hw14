...
-   **UsbR-1.** Система предоставляет простой интерфейс для работы с текстовыми командами

### 14. Требования к производительности / Performance Requirements

-   **Perf_Req-1.** Все сетевые операции (например, передача данных между сервером и клиентами) должны обрабатываться с задержкой не более 1 секунды.
-   **Perf_Req-2.** Система должна обрабатывать команды пользователя с задержкой не более 0.5 секунды.
-   **Perf_Req-3.** Сервер должен обслуживать до N ([LIM-4](#7-ограничения--limitations)) одновременных подключений без заметной деградации производительности.
...
1.  Возможности по доступу (accessing capabilities):
...
2.  Объекты данных и их взаимосвязи
...
4.  Требования к хранению данных (data retention requirements):
